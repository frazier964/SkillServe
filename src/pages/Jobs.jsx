      ...
      <div className="w-full max-w-6xl mx-auto p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-white/70">Browse job postings. Use filters to narrow results.</p>
          </div>
          <div>
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {showPriceRange && (
              <div className="mt-2 p-3 bg-white/5 rounded shadow-md border border-white/10 w-72">
                <div className="text-sm text-white/80 mb-2">Price range (min / max)</div>
                <div className="flex gap-2">
                  <input value={priceMinFilter} onChange={(e) => setPriceMinFilter(e.target.value)} placeholder="min" className="p-2 rounded w-1/2 bg-white/10 text-white" />
                  <input value={priceMaxFilter} onChange={(e) => setPriceMaxFilter(e.target.value)} placeholder="max" className="p-2 rounded w-1/2 bg-white/10 text-white" />
                </div>
          ...
          {filtered.map((job, i) => (
            <div key={i} className="bg-linear-to-r from-white/5 to-white/3 p-4 rounded-xl border border-white/10 flex items-start justify-between hover:scale-105 transform-gpu transition-all duration-200 shadow-sm" role="button" tabIndex={0} onClick={() => navigate(`/jobs/${i}`)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') navigate(`/jobs/${i}`); }} aria-label={`Open job ${job.title}`}>
              ...
              <div className="flex flex-col items-end gap-2">
                <button onClick={(ev) => { ev.stopPropagation(); handleDelete(i); }} className="px-3 py-1 rounded bg-red-600 text-white">Remove</button>
                <button onClick={(ev) => { ev.stopPropagation(); navigate(`/jobs/${i}`); }} className="px-3 py-1 rounded bg-blue-500 text-white">Apply</button>
              </div>
            </div>
          ))}
        </div>
